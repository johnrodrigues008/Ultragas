...
<path d="m13.05 24.637c2.4206 0 4.7869-0.7178 6.7995-2.0626 2.0127-1.3448 3.5814-3.2563 4.5077-5.4926 0.9263-2.2364 1.1687-4.6972 0.6964-7.0713-0.4722-2.3741-1.6378-4.5548-3.3495-6.2664-1.7116-1.7116-3.8923-2.8773-6.2664-3.3495-2.3741-0.47224-4.8349-0.22987-7.0713 0.69646-2.2364 0.92633-4.1478 2.495-5.4926 4.5077-1.3448 2.0127-2.0626 4.3789-2.0626 6.7995 0 3.2459 1.2894 6.3589 3.5847 8.6541 2.2952 2.2953 5.4082 3.5847 8.6542 3.5847zm-1.5776-19.215 6.9761 7.0569-6.9895 6.9088-1.7135-1.7404 5.2492-5.1917-5.2626-5.3202 1.7403-1.7134z" fill="#fff"/>
  ...
  function connectForms() {
    ...
    let rckNext = document.getElementById('rck-next');

    function verificarCampos() {
      if (
        (inputNome.value !== '' && inputSobrenome.value !== '' && inputEmail.value !== '' && inputTelefone.value !== '') &&
        (inputCasa.checked || inputEmpresa.checked)
      ) {
        rckNext.disabled = false; // Permitir o clique no link
        ...
        rckNext.disabled = true; // Desabilitar o clique no link
      }
    }

    inputNome.addEventListener('input', verificarCampos);
    inputSobrenome.addEventListener('input', verificarCampos);
    inputEmail.addEventListener('input', verificarCampos);
    inputTelefone.addEventListener('input', verificarCampos);
    inputCasa.addEventListener('change', verificarCampos);
    inputEmpresa.addEventListener('change', verificarCampos);

    // Prev
    let rckPrev = document.getElementById('rck-prev');
    rckPrev.addEventListener('click', () => {
